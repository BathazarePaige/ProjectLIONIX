...
import { User, Mail, Phone, Globe, Trophy, AlertCircle, Save, X, CheckCircle } from 'lucide-react';
        ...
        <div className="bg-lionix-darkest border border-gray-700/50 rounded-2xl p-8">
          {loading ? (
            ...
            <div className="divide-y divide-gray-700/50">
              ...
              <div className={`flex items-start gap-4 py-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
                <Mail className="w-5 h-5 text-lionix-gray mt-1" />
                <div className={`flex-1 ${isRTL ? 'text-right' : ''}`}>
                  <p className="text-sm text-lionix-light-gray font-noto">{t('emailAddress')}</p>
                  <p className="text-base text-white font-noto font-medium">{user?.email || 'N/A'}</p>
                </div>
              </div>
          ...
          <div className={`flex gap-4 mt-8 ${isRTL ? 'flex-row-reverse' : 'justify-end'}`}>
            <button onClick={handleCancel} className="flex items-center gap-2 bg-lionix-darker text-white font-noto font-medium text-sm py-2 px-4 rounded-lg hover:bg-gray-700 transition-colors">
              <X className="w-4 h-4" />
              {t('cancel')}
            </button>
